defmodule CashFlowExWeb.Errors do
  alias Ecto.Changeset
  alias CashFlowExWeb.Errors.View
  ...
  defp translate_error({message, options}) do
    if options[:count] do
      Gettext.dngettext(CashFlowEx.Gettext, "ecto", message, message, options[:count], options)
    else
      Gettext.dgettext(CashFlowEx.Gettext, "ecto", message, options)
    end
  end
  ...
  defp convert_error_field({field, errors}, memo, schema) when is_list(errors),
    do: memo ++ Enum.flat_map(errors, &convert_error_subfield(&1, field, [], schema))

  defp convert_error_field({field, errors}, memo, schema) when is_map(errors),
    do:
      memo ++
        Enum.flat_map(Map.keys(errors), &convert_error_subfield(&1, field, errors[&1], schema))
